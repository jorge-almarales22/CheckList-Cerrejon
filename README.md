...
La tabla de incorporaciones incluye un **buscador global** a la izquierda del panel de botones (a la derecha quedan "Crear Nueva Incorporación", "Nuevas Solicitudes" y "Solo con Alertas"). Busca solo en la **informacion visible en la tabla** (nombre, plan esperado, completado real, gerencia, superintendencia, tipo, equipos, fechas de inicio/fin y creador), no dentro de las tareas ni comentarios. Aunque la tabla este paginada, el buscador recorre **toda la lista** de incorporaciones (no solo la pagina actual). Al escribir una palabra (ej. "taller"), la tabla se filtra automaticamente en tiempo real mostrando solo las incorporaciones que la contienen en algun campo visible.
