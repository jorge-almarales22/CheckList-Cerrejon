...
const CheckListDetalle = ({ checklistId, onAtras, role, currentUser, theme }) => {
    ...
    const manejarIrRepositorio = async (itemId) => {
        ...
        // No existe: SweetAlert2 con opciones.
        const { value: formValues, isDenied, isDismissed } = await Swal.fire({
            title: 'Carpeta de tarea no encontrada',
            html: `
                <p class="text-sm text-gray-600 mb-4">
                    Esta tarea tiene evidencias en la raíz de la incorporación pero aún no tiene su carpeta física creada.
                </p>
                <div class="flex items-center justify-center gap-2 mt-4 text-sm text-gray-700">
                    <input type="checkbox" id="swal-mover-legacy" class="rounded border-gray-300 text-amber-600 focus:ring-amber-500">
                    <label for="swal-mover-legacy">Mover automáticamente los archivos legacy a la nueva carpeta</label>
                </div>
            `,
            icon: 'warning',
            showCancelButton: true,
            ...
            denyButtonText: 'Solo abrir raíz',
            cancelButtonText: 'Cancelar',
            confirmButtonColor: '#d97706',
            denyButtonColor: '#64748b',
            cancelButtonColor: '#ef4444',
            preConfirm: () => ({
                moverLegacy: document.getElementById('swal-mover-legacy')?.checked || false
            })
        });

        if (isDismissed) return; // Cancelar
        if (isDenied) {
            // Solo abrir raiz en SharePoint.
            window.open(`${AC_HOST}${raizUrl}`, '_blank');
            return;
        }
        if (formValues) {
            await crearCarpetaRepositorio(itemId, formValues.moverLegacy);
        }
    };
    ...
    const crearCarpetaRepositorio = async (itemId, moverLegacy, abrirPestana = true) => {
        ...
        try {
            ...
            if (moverLegacy) {
                ...
                // SweetAlert2 bloqueante con spinner y progreso dinamico.
                Swal.fire({
                    title: 'Organizando archivos...',
                    html: 'Iniciando migración...',
                    allowOutsideClick: false,
                    allowEscapeKey: false,
                    showConfirmButton: false,
                    didOpen: () => Swal.showLoading()
            ...
            await Swal.fire({
                icon: 'success',
                title: '¡Listo!',
                text: 'Carpeta creada y archivos organizados correctamente.',
                timer: 1800,
                showConfirmButton: false
            });
