...
const CheckListDetalle = ({ checklistId, onAtras, role, currentUser, theme }) => {
    ...
    // Progreso de la migracion de archivos legacy: { actual, total } | null.
    const [progresoMigracion, setProgresoMigracion] = useState(null);
        ...
        try {
            ...
            if (info.moverLegacy) {
                ...
                for (let i = 0; i < total; i++) {
                    const f = legacy[i];
                    setProgresoMigracion({ actual: i + 1, total });
                    try {
                ...
                <div className="flex flex-col md:flex-row gap-3 flex-1 w-full">
                    <div className="flex flex-col w-full md:w-1/3">
                        ...
                        <div className="relative">
                            <svg className="w-3 h-4 absolute left-3 top-1/2 -translate-y-1/2 text-slate-400 pointer-events-none" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="m21 21-4.35-4.35m1.35-5.15a6.5 6.5 0 1 1-13 0 6.5 6.5 0 0 1 13 0Z" /></svg>
                            <input
                                type="text"
                                placeholder="Buscar descripcion, responsable, tarea..."
                                value={busquedaGeneral}
                                onChange={(e) => setBusquedaGeneral(e.target.value)}
                                className={`${inputClasses} pl-9 text-xs font-semibold`}
                            />
                        </div>
                    </div>
                ...
                >
                    ...
                    >
                        ...
                        <div className="flex flex-col gap-2">
                            ...
                            <div className={`flex items-center justify-between gap-3 p-2 rounded-lg border ${theme==='dark'?'bg-slate-950/40 border-slate-700':'bg-slate-100 border-slate-300'}`}>
                                <label className="flex items-start gap-2 text-[10px] font-semibold cursor-pointer flex-1 min-w-0">
                                    <input
                                        type="checkbox"
                                        ...
                                        disabled={creandoCarpetaRepo}
                                    />
                                    <span className="leading-tight">Mover automáticamente los archivos legacy a la nueva carpeta</span>
                                </label>
                                <button
                                    onClick={crearCarpetaRepositorio}
                                    disabled={creandoCarpetaRepo}
                                    className="flex items-center justify-center gap-2 text-[11px] font-bold px-3 py-2 rounded-lg bg-amber-600 hover:bg-amber-500 text-white border border-amber-400/30 transition-colors disabled:opacity-50 disabled:cursor-wait shrink-0"
                                >
                                    {creandoCarpetaRepo && (
                                        <span className="w-3.5 h-3.5 border-2 border-white border-t-transparent rounded-full animate-spin" />
                                    )}
                                    {creandoCarpetaRepo ? 'Creando...' : 'Crear carpeta'}
                                </button>
                            </div>
                            {/* Progreso de la migracion de archivos legacy */}
                            {progresoMigracion && (
                                <div className="flex items-center gap-2 text-[10px] font-bold text-amber-600 dark:text-yellow-400">
                                    <span className="w-3 h-3 border-2 border-amber-500 border-t-transparent rounded-full animate-spin shrink-0" />
                                    Moviendo archivo {progresoMigracion.actual} de {progresoMigracion.total}...
                                </div>
                            )}
                        </div>
                    </div>
